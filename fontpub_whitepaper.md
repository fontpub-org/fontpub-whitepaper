...
The protocol is composed of four interoperable files — `fontpub.json`, `fontpub-keys.json`, `fontpub-proof.txt`, and `fontpub-curation.json` — which together establish a verifiable relationship between a font, its publisher, and its source domain. Clients can independently install and verify fonts using standard web formats and modern cryptographic primitives (HTTP, JSON, and Ed25519).
